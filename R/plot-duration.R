...
plot_duration <- function(datavyu_object) {
  ...
  duration_df <- stringr::str_split(datavyu_object$sum_duration, ":") %>%
    as.data.frame()

  names(duration_df) <- stringr::str_c("row", 1:length(datavyu_object$sum_duration))

  datavyu_object$sum_duration <- lubridate::duration(hour = as.numeric(duration_df[1, ]),
