...
The [{here} package](https://here.r-lib.org/) is used to specify file paths:

```{r}
library(here)

summary_of_files <- datavyu_col_search(here("irr-data", "datavyu_output_11-16-2020_14-13"))

summary_of_files

summary_of_files %>% 
  distinct(column, codes)
```

To save on typing, the directory can be set for an entire R session via the following:
...
We'll do a ["full" join](https://dplyr.tidyverse.org/reference/join.html), retaining all time stamps for both files. First, we must rename one (or both) of the two `code` columns. Having done this, we can easily compare the two once joined:
